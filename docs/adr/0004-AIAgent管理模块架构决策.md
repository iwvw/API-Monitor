...
实例是「一台已装 Agent 的主机 + 一种 AI Agent 类型 + 一个本地端口」，是**平台共享资源，不归属任何用户**。

1. 实例由面板管理员统一登记；AI Agent 用户不创建实例，只被授权使用其中的若干实例。
2. 用户与实例是多对多授权：`aiagent_instance_grants(instance_id, user_id)`。**默认不授权**，需在用户管理里逐个勾选。
3. 唯一约束 `(server_id, provider)`，同一主机同一 Provider 只登记一次（实例已是平台资源，不再按用户区分）。
4. **端口每实例可配**，默认取 Provider 默认端口（OpenCode 为 `4096`），因为实际部署端口可能被占用或自定义。
5. 网关与实例相关接口按授权校验：管理员全权，用户仅可访问被授权的实例；未授权与不存在统一按「不存在」返回，避免暴露实例存在性。

历史演进（实现期调整）：第一版模型是「实例归属一个用户（`user_id`）、唯一约束 `(user_id, server_id, port)`」。后因需求变为「平台统一登记实例、用户按需选择可用实例」而改为当前模型。升级时旧库的 `user_id` 会一次性转为授权记录后删除该列，见 `schema.go` 的 `migrateInstanceOwnership`。

补充（实现期安全加固）：**把一台新主机首次纳入清单属于管理员动作**。`handleServers` 与实例相关接口都按调用方授权过滤，避免任意登录用户枚举全舰主机并探测其端口状态。
...
模块不建立角色、团队等权限模型。实例是平台共享资源，用户通过逐条授权获得可用实例，管理员是全权。这与需求「平台统一登记实例、用户按需选择」一致，也与云厂商模块「不做多租户 RBAC」的既有决策一致。

## 后果

- 优点：一个用户一个入口即可管理跨多台机器的多个 AI Agent；不暴露公网端口；用户体系与面板安全域隔离；实例作为平台资源共享给多个用户；Provider 抽象使后续接入成本低；复用既有 Agent 通道与 broker 机制，新增协议面小。
