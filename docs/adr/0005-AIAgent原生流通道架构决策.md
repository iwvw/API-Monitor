...
### 9. 实测：每请求建流的延迟代价（2026-09）

第 8 节第 3 条规划的多路复用尚未实施，此处记录当前实现的实测数字，作为后续是否启动该改造的判据。

测量环境：客户端经公网访问部署在云端的 panel，经网关转发到主机 Agent（跨地域）。

| 请求 | 平均延迟 | 说明 |
| --- | --- | --- |
| 本地直连 opencode `/path` | 74 ms | 无网关的基线 |
| 面板接口（不建流） | 562 ms | 网络 RTT + 面板处理 |
| 网关 `/path`（建流） | 1354 ms | 同上再加一次建流 |
| 网关 `/config`（建流） | 1901 ms | 同上 |

结论：**每请求建流的固定开销约 790 ms**，是同一请求在本地耗时的十倍以上。连发 10 次无长尾（min 1248 / max 1433），说明这是稳定结构性成本而非抖动。

流式输出的首 token 实测：本地直连 477 ms，经网关 3082 ms。建流约占其中的四分之一，其余为跨地域往返与模型自身首 token。

同期已消除的一项非结构性开销：`engineio.go` 的 `writeLoop` 原先以 `time.Sleep(50ms)` 忙等轮询发送队列，每条控制通道消息平均多等 25 ms 且持续空转。已改为 channel 通知（`notify` 字段 + `notifyPendingMessagesLocked`），控制通道消息送达不再受轮询周期影响。该项不改变建流成本，只是让任务下发不再额外叠加延迟。

多路复用改造（第 8 节第 3 条）尚未启动：需改动 Rust Agent 协议并重新发布、两端联调，属独立改造范围，收益为消除上述约 790 ms 的每请求开销。
