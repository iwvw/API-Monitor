...
- 400/401/403 等 4xx 不换号：那是请求或凭据侧问题，换号重发同样会失败（401 走既有 token 刷新路径）。
  **例外**：4xx 正文里若带**模型级限流**文案，则视为可重试（见下节）。

### 模型级限流：账号 × 模型，不是账号（已实现）

上游会对**某个账号的某个模型**单独限流，文案形如（实测原文）：
...
这与上节的「账号冷却」是**两件事**，不能复用同一套机制，理由有三：

1. **维度不同**：这是「账号 × 模型」。若按账号冷却处理，会把该账号**还能用的其它模型**
   一起误伤（用户明明可以换模型继续）；反之，若什么都按模型记，瞬时故障又会让
   某个模型被无谓地晾很久。
2. **窗口量级不同**：重置窗口实测可达**数天**，而账号冷却固定 5 分钟 —— 5 分钟后再试
   只会继续撞墙，并对上游产生无谓请求。
3. **调用方需要的信息不同**：用户真正要知道的是「**到几点能用**」，而不是一个笼统的 502。

因此新增 `ratelimit.go`，单独维护一张**模型限流簿**（`modelLimitMu` / `modelLimits`，
键 = `accountID + NUL + model`，纯内存、重启即清空，与账号冷却同理由）：

- **识别（三条路径，都不依赖 HTTP 状态码）**。上游失败时未必用 4xx，实测会用
  **HTTP 200 + `{code,msg}` 信封**，若不解出来，中继会把「空回答」当成功透给下游
  （下游收到一个没有内容的 completion，既不是错误也无从分辨）。故统一走
  `classifyUpstreamPayload`：
  - HTTP ≥ 400 的正文；
  - **SSE 流的开头**——`streamToClient` 在 `WriteHeader` **之前**先窥探开头若干行
    （跨过空行/心跳）。这是唯一的拦截窗口：响应头一旦写出就再也无法换号重试；
  - 非流式聚合路径的每个 chunk（聚合前拦下，此刻同样尚未写出任何字节）。
- **严格 / 宽松两条判据**，防误判：
  - 错误信封的 `msg` 是**错误说明而非模型正文**，故**有措辞即可**判限流；无重置时刻时
    回落 `modelLimitDefault`（30 分钟）。
  - **自由文本**必须「限流措辞 + 可解析时刻」同时命中，且**不含 `choices`**（正常 chunk
    一定带 choices）——否则用户问一句「rate limit 是什么、比如 2026-09-12 17:22:50」
    就能把一段正常回答误判成限流。
  - `envelopeError` 判定刻意保守：必须带 `code` 且**非 0**（0 是成功码）、且不带 `choices`。
- **重置时刻按站点时区解释**：上游文案用的是站点时区，按 UTC 解释会整整差 8 小时
  （限流提前/延后解除）。坏时刻一律有归宿：零值/已过去 → 兜底 30 分钟；
  离谱未来 → 封顶 30 天（`clampModelLimitUntil`），绝不让一个解析串味的时刻长期锁死账号。
- **选号按模型过滤**：`pickLeastConsumed(accounts, model)` 与 `refreshFirstStaleAccount`
  都会跳过「该账号 × 该模型」限流中的账号 —— 同一账号在**别的模型**上照常可选。
- **回落与报错**：
  - 首号撞限流 → 记入限流簿（**不进账号冷却**）→ 换下一个号；别的号正常则用户无感；
  - 该模型在**所有**账号上都限流 → **完全不再请求上游**，直接回 `429 rate_limit_error`
    并带上**最早恢复时刻**；
  - 因限流失败的兜底错误也回 429（而非 502），错误体带「该模型将于 … 恢复可用」。
- **可观性（三个面，回答三个不同的问题）**：
  - `GET /api/workbuddy/v1/_diag` 的 `live.modelLimits` 列出当前生效记录 + 最近一次上游原文
    （`rateLimitSample` / `rateLimitSampleAt`）——**排障**用。
  - **账号列表**的 `AccountView.limitedModels` 下发每个账号被限流的模型与恢复时刻，
    插件页在 token 徽标下显示「限流 N 模型」（悬浮看时刻）——回答「**谁**被限流」。
    账号的 `available` 字段**不受影响**：限流只是部分模型暂时不可用。
  - **模型列表**的每项带 `limit` 汇总（`handleModels` 计算）：`limited` / `usable` /
    `allLimited` / `nextRecoveryAt` —— 回答「**这个模型还能不能用**」。模型行在 id 下方
    显示徽标：全部账号都限流时为 `danger`「限流中」，部分限流时为 `warning`「限流 x/y」，
    都带最早恢复时刻。两个面的信息不可互替：账号表看得到「谁」却看不出「模型是否整体不可用」。
  - `limited`/`usable` 的**分母只算可用账号**（`accountAvailable`）：停用/无凭据/token 已过期的
    账号本就不参与选号，若算进分母会让「全部限流」永远不成立（提示永远不出现）；
    反过来，一个早已停用的账号也不该让某模型永远显示「限流中」。

> 与 CodeBuddy 的关系：本插件请求上游时用的就是 CodeBuddy 客户端身份
> （`CLI/… CodeBuddy/…` + Referer `codebuddy.cn`），因此天然吃「CodeBuddy 那一份额度」。
> WorkBuddy / CodeBuddy 的额度限制相互独立、积分共享，一侧撞限流时另一侧仍可用 ——
> 这也是**多账号 + 模型级回落**值得做细的原因。
