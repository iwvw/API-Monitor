...
1. ~~**多账号调度**：现为「首个可用 / 首个可刷新」，无轮询与冷却。~~ **已定案**：
   采用「**站点时区今天已消耗 credit 最少**」选号（见 §14）。仍缺 429/配额超限的冷却与失败换号。
...
上游返回 **429 / 配额超限**（资料里提到额度类错误码，如 `14003`）时，当前**直接 502 抛给调用方**，
**不会**把该账号冷却并换号重试。要做的话就在 `chatCompletions` 的错误分支加上
「标记账号冷却 + 换号重试一次」——冷却状态可以复用 `Account.LastError` 与一个 `CooldownUntil` 字段。
