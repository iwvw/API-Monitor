...
- 例外：模型网关的网关日志 tab（`src/js/pages/openai/GatewayLogsTab.jsx`）在整页滚动模块内做了表格内部滚动。该 tab 的日志列表需要固定表头与固定分页控件，跟随整页滚动时看不到分页。实现方式：表格外层 `LayerCard` 按视口高度扣减（`h-[calc(100dvh-184px)]`，含 `min-h-64` 兜底），表头 sticky 吸顶、行在容器内滚动，分页固定在卡片底部。代价：偏移量 184 由顶栏 58 + 吸顶 tab 栏 58 + PageStack 间距 16 + 筛选行 28（sm 控件 `h-7`）+ 卡片间距 12 + 底部 gutter 12 累加得出，属硬编码；改动顶栏、tab 栏或筛选行高度时必须同步调整。要消除该例外，应把 `openai` 整体迁入 viewport 工作区，并一并处理其余 tab 的内部滚动，不要只改这一个 tab 的模式。
