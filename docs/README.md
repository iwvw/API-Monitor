...
- `kumo-design-违规审计清单.md` / `frontend组件一致性分析报告.md`（一次性审计）
- `Go后端迁移状态.md`（迁移历史，迁移已完成）
- `handoff-zcode-ask-ai-sidebar.md`（交接）
- `多Agent协作登记.md`（已废弃，协作改由 GitHub Issues 跟踪，见 `CONTEXT.md`）
- `Cloudflare看板设计分析.md`（一次性分析，建议已随卡片库/看板落地）

`docs/prd/` 下的管理 AI 系列、文档编辑器、提示词库、定时任务/工作流、Draw.io、DevOps GitHub、Oracle/GCP 模块等 PRD 均已实现上线，属历史需求文档，仅在需要回溯原始需求时查阅。
