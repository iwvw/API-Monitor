...
- **🤖 AI API 网关**: OpenAI 兼容代理、管理 AI、插件中心（代理池 / Antigravity / DS2API）
- **☁️ 云服务管理**: Cloudflare、阿里云、腾讯云、Oracle OCI、GCP、华为云、Koyeb、Fly.io、M365、GitHub、Docker Hub
- **🖥️ 主机监控**: 实时终端、Docker 管理、系统指标监控、转发中心
- **🔧 工具箱**: TOTP 双因子认证、文件分享、可用性监测、通知系统、提示词库、Draw.io、定时任务/工作流
...
- **当前状态**: 路由清单由 `backend-go/internal/manifest/manifest.go` 管理，当前 route inventory 为 369 条 Go-owned 路由。
...
| **React** | 19.2.8 | UI 框架 |
| **Vite** | 8.2.2 | 构建工具 |
| **Tailwind CSS** | 4.3.3 | 样式框架 |
| **Zustand** | 5.0.15 | 状态管理 |
| **@cloudflare/kumo** | 2.13.2 | 组件库 |
...
graph TB
    ...
    subgraph "业务模块"
        ...
        E2[OpenAI 网关/插件]
        E3[Server Monitor]
        E4[Aliyun/Tencent/Oracle/GCP/Huawei]
        E5[TOTP/Filebox/Uptime/Cron]
    end
...
- `backend-go/internal/proxypool/`（代理池）
- `backend-go/internal/antigravity/`、`backend-go/internal/ds2api/`（网关插件）
- `backend-go/internal/adminai/`（管理 AI）

**核心功能**:
- OpenAI 兼容 API 端点管理与日志统计
- 插件中心（代理池轮询出口、Antigravity / DS2API 兼容网关）
- 请求日志、SSE 流式与用量统计（小时/分钟粒度）
- 模型列表聚合
- 管理 AI：会话、SSE 工具调用、审批与审计
...
GET  /api/admin-ai/*             # 管理 AI（会话/审批/审计）
```
...
│   ├── openai/                  # OpenAI 网关
│   ├── proxypool/               # 代理池
│   ├── antigravity/             # Antigravity 网关插件
│   ├── ds2api/                  # DS2API 网关插件
│   ├── adminai/                 # 管理 AI
│   ├── aliyun/ tencent/ koyeb/ flyio/ m365/      # 云厂商
│   ├── oracle/ gcp/ huawei/ onepanel/            # 云厂商
│   ├── github/ dockerhub/                       # DevOps 观察
│   ├── totp/ filebox/ uptime/ notification/     # 工具箱
│   ├── cronjobs/ prompts/ bookmarks/ drawio/    # 工具箱
│   ├── subscription/ subscriptionledger/        # 订阅分发
│   ├── managedproxy/ system/ backup/            # 系统能力
│   └── ...                     # 其余 50+ 个 internal 包
└── go.mod
```
...
参考 `.env.example` 与根目录 `README.md` 的配置表（`ENCRYPTION_KEY` 生产必填且至少 32 位）：
...
完整清单见 [新模块接入指南](../guides/新模块接入指南.md)（模块 ID、后端模块、路由注册、前端页面、设置同步、UI 约束、空态与验收），逐步代码模板见 [开发指南·添加新的 API 路由](../guides/开发指南.md)。本节只说明各层在这条链路上的落点：

| 层 | 落点 |
| --- | --- |
| 后端模块 | `backend-go/internal/<module>/`（service.go / service_test.go / types.go） |
| 路由清单 | `backend-go/internal/manifest/manifest.go`（Owner/Auth/ResponseMode/MatchMode） |
| 路由分发 | `backend-go/internal/server/server.go` 的 `serveGoRoute` case |
| 前端页面 | `src/js/pages/<ModuleName>Page.jsx` |
| 模块注册 | `src/js/store.js` 的 `MODULE_CONFIG` + `src/js/components/MainLayout.jsx` 渲染映射 |

### 前端开发规范

组件选择与硬约束见 `docs/standards/Kumo UI 规则.md`，布局约定见 `docs/standards/前端布局约定.md`。本节只列本仓库的状态与请求写法：
...
命名与错误处理约定见 [开发指南·代码规范](../guides/开发指南.md)，模块结构与目录模板见 [新模块接入指南](../guides/新模块接入指南.md)。本仓库的固定写法：

- 模块目录固定 `service.go` / `service_test.go` / `types.go` 三件套。
- handler 内错误统一走 `response.Error(w, http.StatusInternalServerError, err.Error())`。
- 数据库访问在模块内部打开与关闭：`db, err := s.store.Open(ctx)` + `defer db.Close()`，不在 dispatch 层散落 SQL。
...
- [架构设计](./设计文档.md)
- [新模块开发指南](../guides/新模块接入指南.md)
- [Kumo UI 规则](../standards/Kumo%20UI%20规则.md)
- [前端布局约定](../standards/前端布局约定.md)
- [Go 后端指南](../guides/GO后端启动指南.md)
- [Go 后端迁移状态](../archive/Go后端迁移状态.md)
...
- ✅ 路由清单刷新至 369 条，补齐云厂商模块（Oracle/GCP/华为云/M365/Onepanel）
- ✅ 文档与技术栈版本对齐到当前依赖（Kumo 2.13.1、React 19.2.8、Vite 8.2.2）
