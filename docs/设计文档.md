...
Go 迁移分支新增 `backend-go/`。当前 Go main 已提供健康检查、迁移状态、静态资源兜底、legacy proxy、退役音乐/OpenList 响应、核心认证路径、settings/system/logs/TOTP/Cron/Filebox/Notification/Uptime/Koyeb/Fly.io/Aliyun/Tencent 纵切面，以及 Cloudflare accounts + DNS + zone resources/templates + Workers 子集；未迁移模块和 Cloudflare Pages/R2/Tunnels 继续由 Node sidecar 承担。
...
- `cloudflare-api`：Cloudflare 账号、域名、DNS、Workers、Pages、R2、Tunnels；当前迁移分支中 accounts CRUD/import/export/verify/token、zones、DNS records、SSL、analytics、purge、DNS templates 和 Workers 已由 Go main 接管，Pages/R2/Tunnels 仍在 Node sidecar。
- `openai-api`：OpenAI 兼容端点、模型、聊天与日志。
- `gemini-cli-api`、`qwen-api`：AI 代理与调用统计。
- `koyeb-api`、`flyio-api`：PaaS 平台状态同步；当前迁移分支中 Koyeb 与 Fly.io 均已由 Go main 接管。
- `totp-api`、`filebox-api`、`uptime-api`、`notification-api`、`aliyun-api`、`tencent-api`：工具与云厂商能力；当前迁移分支中 Aliyun 与 Tencent 已由 Go main 接管；`openlist-api` 已按后端迁移决策退役。
...
详细规则见 [Kumo UI 迁移规则.md](./Kumo UI 迁移规则.md)。
...
- SSH/终端、Docker 更新、Agent 遥测、Cloudflare Pages/R2/Tunnels 和腾讯云等仍需要真实外部环境持续回归；Koyeb/Fly.io/Aliyun/Tencent/Cloudflare accounts + DNS + zone resources/templates + Workers 已有 fake-provider smoke，但真实账号仍需人工验证。
