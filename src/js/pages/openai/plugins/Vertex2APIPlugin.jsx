...
import { AppCard, EmptyState, PageStack, FieldRow } from '../../../components/ui/AppPrimitives.jsx';
...
export function Vertex2APIPlugin({ onBack }) {
  ...
  // 未保存的字段暂存：Textarea/Input 用本地态，避免每键触发全量保存。
  ...
  const applyDraft = () => {
    if (!draft) return;
    update({ ...draft, manualProxies: manualProxyText.split('\n').map(s => s.trim()).filter(Boolean) });
    setDraft(null);
  };
  ...
  return (
    <PageStack viewport>
      <div className="flex min-h-0 flex-1 flex-col gap-4">
        ...
        <AppCard title="Vertex to API" description="内嵌免费 Gemini 中继（OpenAI 兼容接口）。测试性子插件，可随时移除。">
        ...
        <AppCard title="接入模型网关端点列表" description="把本插件注册为模型网关的一个端点（出现在「API 端点」页）。接入后外部客户端通过网关 /v1 路由到本中继，自动复用网关的密钥、路由、日志与代理池。">
          <div className="flex flex-wrap items-center gap-3">
            {linkState?.linked ? (
              <>
                <span className="inline-flex items-center gap-1.5 rounded-md border border-kumo-success/40 px-2.5 py-1 text-xs text-kumo-success">
                  ● 已接入 {linkState.name || 'Vertex to API'}
                </span>
                <span className="text-xs text-kumo-subtle break-all">{linkState.baseUrl}</span>
              ...
              <>
                <span className="text-xs text-kumo-subtle">未接入。接入后端点地址：{linkState?.baseUrl || '计算中...'}</span>
                <div className="ml-auto">
          ...
          {!settings?.enabled && (
            <p className="mt-2 text-xs text-kumo-warning">提示：需先启用中继才能接入网关端点。</p>
          )}
        </AppCard>

        <AppCard title="出口代理" description="出网到 Google 时使用的代理。可复用模型网关已有端点的代理池，或手动填写代理列表（每行一个，支持 http/https/socks5）。">
        ...
        <AppCard title="高级" description="中继运行参数。">
          <Button size="sm" variant="ghost" onClick={() => setAdvancedOpen(o => !o)}>
            {advancedOpen ? '收起高级参数' : '展开高级参数'}
          </Button>
          {advancedOpen && (
            <div className="mt-2">
              {numberField('requestTimeout', '请求超时（秒）', 1, 1800)}
              {numberField('maxRetries', '最大重试次数', 0, 10)}
              {numberField('streamIdleTimeoutSeconds', '流式空闲超时（秒）', 1, 300)}
              {numberField('maxN', '最大候选 n', 1, 8)}
