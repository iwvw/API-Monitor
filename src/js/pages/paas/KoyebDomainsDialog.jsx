...
import { Plus, RefreshCw, Trash } from '../../components/Icons.jsx';
...
export default function KoyebDomainsDialog({ koyebDomainsTarget, setKoyebDomainsTarget, koyebNewDomain, setKoyebNewDomain, addKoyebDomain, koyebDomainsLoading, koyebDomainsError, koyebDomains, getKoyebStatusText, refreshKoyebDomain, isArmed, deleteKoyebDomain }) {
  return (
      <LayerDialog.Root open={!!koyebDomainsTarget} onOpenChange={(open) => { if (!open) setKoyebDomainsTarget(null); }}>
        <LayerDialog.Content size="base">
          <LayerDialog.Title>域名管理</LayerDialog.Title>
          <LayerDialog.Description>{koyebDomainsTarget ? (koyebDomainsTarget.app ? `应用 ${koyebDomainsTarget.app.name}` : '组织全部域名') : ''}</LayerDialog.Description>
          <LayerDialog.Body>
