...
export const sshSplitMethods = {
  ...
  addToSplitView(sessionId, position = 'right') {
    ...
    if (this.sshViewLayout === 'single') {
      this.visibleSessionIds = this.activeSSHSessionId ? [this.activeSSHSessionId] : [];
    }
  ...
  _updateLayoutMode(position = 'right', targetIndex = -1) {
    ...
    if (count <= 1) {
      ...
      this.sshViewLayout = position === 'top' || position === 'bottom' ? 'split-v' : 'split-h';
      this.sshSplitSide = '';
    } else if (count === 3) {
  ...
  onDrop(targetId, position) {
    ...
    console.log(
      `[SSH Split] Drop: dragged=${draggedId}, target=${targetId}, pos=${effectivePosition}`
    );
  ...
  _insertInSplit(sessionId, targetId, position) {
    ...
    if (targetIndex !== -1) {
      insertAt = position === 'right' || position === 'bottom' ? targetIndex + 1 : targetIndex;
    } else {
