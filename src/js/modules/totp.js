...
export const totpMethods = {
  ...
  openAddTotpModal() {
    ...
    const defaultMode = this.totpSettings.lockInputMode
      ? this.totpSettings.defaultInputMode
      : 'scan';
    ...
    if (
      this.totpAddTab === 'scan' &&
      (!this.totpSettings.lockInputMode || this.totpSettings.defaultInputMode === 'qr')
    ) {
  ...
  async startQrScan() {
    ...
    this.$nextTick(async () => {
      try {
        ...
        this.qrError =
          friendlyMsg + ' (' + (err.name || 'UnknownError') + '): ' + (err.message || '无详细信息');
  ...
  filteredTotpAccounts() {
    ...
    if (this.totpSearchQuery) {
      ...
      accounts = accounts.filter(
        acc =>
          acc.issuer?.toLowerCase().includes(query) || acc.account?.toLowerCase().includes(query)
      );
    }
