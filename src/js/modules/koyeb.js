...
export const koyebMethods = {
  ...
  async importKoyebAccounts() {
    ...
    input.onchange = e => {
      ...
      reader.onload = event => {
        ...
        // 自动跳转到批量添加面板
        this.$nextTick(() => {
          const batchPanel = document
            .querySelector('.panel-title .fa-layer-group')
            ?.closest('.panel');
          if (batchPanel) {
  ...
  async deleteKoyebService(account, app, service) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (appIndex !== -1) {
          const serviceIndex = account.projects[appIndex].services.findIndex(
            s => s._id === service._id
          );
          if (serviceIndex !== -1) {
  ...
  startEditKoyebServiceName(service) {
    ...
    this.$nextTick(() => {
      ...
      const activeInput = document.querySelector(
        '.service-name-input:not([style*="display: none"])'
      );
      if (activeInput) {
        activeInput.focus();
        activeInput.select();
    ...
    this.openLogViewer({
      ...
      fetcher: async () => {
        const response = await fetch(
          `/api/koyeb/services/${service._id}/logs?accountId=${account.id}`
        );
        const result = await response.json();
  ...
  async fetchKoyebServiceMetrics(account, service) {
    try {
      ...
      const cpuRes = await fetch(
        `/api/koyeb/services/${service._id}/metrics?accountId=${account.id}&name=CPU_TOTAL_PERCENT`
      );
      const cpuData = await cpuRes.json();

      // 获取内存指标
      const memRes = await fetch(
        `/api/koyeb/services/${service._id}/metrics?accountId=${account.id}&name=MEM_RSS`
      );
      ...
      if (cpuData.success && memData.success) {
        // 提取最新值
        const latestCpu =
          cpuData.metrics?.[0]?.data?.[cpuData.metrics[0].data.length - 1]?.[1] || 0;
        const latestMem =
          memData.metrics?.[0]?.data?.[memData.metrics[0].data.length - 1]?.[1] || 0;
  ...
  async fetchKoyebServiceInstances(account, service) {
    ...
    try {
      const response = await fetch(
        `/api/koyeb/services/${service._id}/instances?accountId=${account.id}`
      );
      const result = await response.json();
