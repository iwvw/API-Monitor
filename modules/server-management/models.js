...
class ServerCredential {
  static getAll() {
    const stmt = getDb().prepare(
      'SELECT * FROM server_credentials ORDER BY is_default DESC, created_at DESC'
    );
  ...
  static setDefault(id) {
    try {
      ...
      // 设置指定凭据为默认
      const result = db
        .prepare('UPDATE server_credentials SET is_default = 1 WHERE id = ?')
        .run(id);
...
class ServerMetricsHistory {
  ...
  static getHistory(options = {}) {
    const { serverId = null, startTime = null, endTime = null, limit = 100, offset = 0 } = options;

    let sql =
      'SELECT h.*, a.name as server_name, a.host as server_host FROM server_metrics_history h LEFT JOIN server_accounts a ON h.server_id = a.id WHERE 1=1';
    const params = [];
