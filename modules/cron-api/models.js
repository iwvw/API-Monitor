...
class CronTaskModel extends BaseModel {
    ...
    updateTask(id, updates) {
        const allowedFields = ['name', 'schedule', 'command', 'type', 'enabled', 'last_run', 'next_run'];
        const data = {};
...
class CronLogModel extends BaseModel {
    ...
    getLogsByTask(taskId, limit = 50) {
        ...
        return db.prepare(`SELECT * FROM ${this.tableName} WHERE task_id = ? ORDER BY start_time DESC LIMIT ?`).all(taskId, limit);
    }
