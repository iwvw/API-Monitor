...
class TotpAccount extends BaseModel {
    ...
    updateAccount(id, updates) {
        const allowedFields = ['otp_type', 'issuer', 'account', 'secret', 'algorithm', 'digits', 'period', 'counter', 'group_id', 'icon', 'color', 'sort_order'];
    ...
    getAllSorted() {
        ...
        const stmt = db.prepare(`SELECT * FROM ${this.tableName} ORDER BY sort_order ASC, created_at ASC`);
        return stmt.all();
    }
    ...
    getByGroup(groupId) {
        const db = require('../../src/db/database').getDatabase();
        const stmt = db.prepare(`SELECT * FROM ${this.tableName} WHERE group_id = ? ORDER BY sort_order ASC`);
    ...
    updateOrder(orderedIds) {
        const db = require('../../src/db/database').getDatabase();
        const stmt = db.prepare(`UPDATE ${this.tableName} SET sort_order = ?, updated_at = ? WHERE id = ?`);
