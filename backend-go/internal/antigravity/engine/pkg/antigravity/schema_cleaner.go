...
func flattenRefs(schema map[string]any, defs map[string]any) {
	flattenRefsDepth(schema, defs, make(map[string]bool), 0)
}

const maxFlattenRefsDepth = 32

// flattenRefsDepth 递归展开 $ref，带环检测与深度上限，防止自引用/循环 $ref 导致无限递归。
func flattenRefsDepth(schema map[string]any, defs map[string]any, seen map[string]bool, depth int) {
	if len(defs) == 0 || depth > maxFlattenRefsDepth {
		return // 无需展开或已达深度上限
	}
	...
	if ref, ok := schema["$ref"].(string); ok {
		...
		if defSchema, exists := defs[refName]; exists {
			if defMap, ok := defSchema.(map[string]any); ok {
				...
				// 递归处理刚刚合并进来的内容（标记已访问，防止 A→B→A 环）
				seen[refName] = true
				flattenRefsDepth(schema, defs, seen, depth+1)
				delete(seen, refName)
			}
		}
	}
	...
	for _, v := range schema {
		if subMap, ok := v.(map[string]any); ok {
			flattenRefsDepth(subMap, defs, seen, depth+1)
		} else if subArr, ok := v.([]any); ok {
			for _, item := range subArr {
				if itemMap, ok := item.(map[string]any); ok {
					flattenRefsDepth(itemMap, defs, seen, depth+1)
				}
			}
		}
	}
