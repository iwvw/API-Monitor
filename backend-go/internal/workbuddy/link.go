...
// disabled_models 列。
//
// 为什么必须同步：网关是按端点这一列做请求拦截的 —— `internal/openai/relay.go` 的
// resolveEndpointModel → isModelDisabled(ep.DisabledModels, requested)。
// 端点 models 列只是展示/刷新用的名单，既不拦请求也不做白名单。
