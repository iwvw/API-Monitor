...
func TestPickLeastConsumed(t *testing.T) {
	...
	acc, _ = s.pickLeastConsumed([]Account{a1, a2}, "hy3", nil)
	if acc.ID != "u1" {
	...
	acc, ok = s.pickLeastConsumed([]Account{disabled, expired, noToken, a2}, "hy3", nil)
	if !ok || acc.ID != "u2" {
		t.Fatalf("应跳过不可用账号选 u2，得到 %v/%v", acc.ID, ok)
	}

	// 全不可用 → false。
	if _, ok := s.pickLeastConsumed([]Account{disabled, expired, noToken}, "hy3", nil); ok {
